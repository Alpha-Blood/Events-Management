      ...
      <div className="mx-auto w-full max-w-7xl px-8 py-12">
        <div className="grid grid-cols-1 justify-between gap-8 md:grid-cols-2">
          <div className="space-y-4">
            <h5 className="text-blue-600 font-bold text-lg">Eventico</h5>
            <p className="text-gray-600 max-w-md">
              Your one-stop destination for discovering and booking amazing events. Find unforgettable experiences that match your vibe.
            </p>
          </div>
          <div className="grid grid-cols-3 justify-between gap-8">
            {LINKS.map(({ title, items }) => (
              <ul key={title}>
                <li className="mb-4 font-bold text-gray-900">{title}</li>
                {items.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="py-1.5 block text-gray-600 hover:text-blue-600 transition-colors"
                    >
                      {link}
                    </a>
                  </li>
        ...
        <div className="mt-12 flex w-full flex-col items-center justify-center border-t border-gray-200 py-4 md:flex-row md:justify-between">
          <p className="mb-4 text-center text-sm text-gray-600 md:mb-0">
